...
class adderror(modulebase):
    ...
    def once(self, state):
        ...
        pad = torch.zeros(rows * cols - n, device=self.device, dtype=torch.bool)

        padded_mask = torch.cat([qmask, pad], dim=0).view(rows, cols)

        row_sum = padded_mask.int().sum(dim=1)
        col_sum = padded_mask.int().sum(dim=0)

        row_count = 1 + int(self.config['row_percent'] * self.config['error_percent'] * n * 2 / cols)
        col_count = 1 + int(self.config['col_percent'] * self.config['error_percent'] * n * 2 / rows)

        row_mask = row_sum > row_sum.topk(row_count).values.min()
    ...
    def preprocess(self, state):
        ...
        error_percent = self.config['error_percent']
        error_percent *= self.config['error_total'] / self.config['concentrate_bit']
        ...
        row_mask, col_mask = state['mask_cache']

        err_mask_row = torch.rand((rows, cols, bit_total), device=self.device) < error_percent * row_percent / (row_mask.float().mean() + 1e-6)
        err_mask_row = (err_mask_row * row_mask.view(-1, 1, 1)).view(-1, bit_total)[:n, :]
        err_mask_col = torch.rand((rows, cols, bit_total), device=self.device) < error_percent * col_percent / (col_mask.float().mean() + 1e-6)
        err_mask_col = (err_mask_col * col_mask.view(1, -1, 1)).view(-1, bit_total)[:n, :]

        err_mask = err_mask_single | err_mask_row | err_mask_col
        #err_mask = torch.rand((n, bit_total), device=self.device) < self.config['error_percent']

        err_mask |= torch.rand((n, bit_total), device=self.device) < self.config['error_percent'] - err_mask.float().mean()
...
class eccerr(modulebase):
    ...
    def process(self, state):
        ...
        slicer = torch.arange(self.config['bitwidth'], device=self.device, dtype=torch.long).view(1, -1)

        sliced = (err.view(-1, 1) & (1 << slicer)).bool()

        numel = sliced.numel()

        rows = math.ceil(numel / self.config['cols'])
        pad = rows * self.config['cols'] - numel
        padded = torch.cat([sliced.view(-1), torch.zeros(pad, dtype=torch.bool, device=self.device)]).view(rows, self.config['cols'])
        idx = padded.max(dim=1).indices

        row_range = torch.arange(rows, device=self.device, dtype=torch.long)

        padded[row_range, idx] = False

        sliced = padded.view(-1)[:numel].view_as(sliced)

        state['err_xor'] = (sliced * (1 << slicer)).sum(dim=1, dtype=torch.short)
...
class wesco_v2_base(modulebase):
    ...
    def __init__(self, **kwargs):
        ...
        self.encoding_count, self.combined_bit_width = self.encoding.shape
    ...
    def combine_neighbour(self, tensor: torch.Tensor):
        raise NotImplementedError

    def split_neighbour(self, tensor: torch.Tensor):
        raise NotImplementedError
    ...
    def process(self, state):
        ...
        sliced_tensor = self.combine_neighbour(sliced_tensor)
        sliced_stuck_at = self.combine_neighbour(sliced_stuck_at)
        sliced_stuck_at_mask = self.combine_neighbour(sliced_stuck_at_mask)

        frow, fcol = self.row(row), self.col(col)
        ...
        # (encoding_count, bitwidth, row, col)
        unmatch_sum = exchanged_stuck_at_mask & (sliced_tensor.view(1, self.combined_bit_width, frow, fcol) ^ exchanged_stuck_at)
        ...
        row_range = torch.arange(frow, dtype=torch.long, device=self.device)
        ...
        injected_sliced_tensor = self.split_neighbour(injected_sliced_tensor.transpose(0, 1)).transpose(0, 1)
...
class wesco_v2_row(wesco_v2_base):

    default_config = {}

    def combine_neighbour(self, tensor: torch.Tensor):
        bit, row, col = tensor.shape
        return tensor.reshape(bit * 2, row // 2, col)

    def split_neighbour(self, tensor: torch.Tensor):
        bit_width, row_half, col = tensor.shape
        return tensor.reshape(bit_width // 2, row_half * 2, col)
...
class wesco_v2_col(wesco_v2_base):

    default_config = {}

    def combine_neighbour(self, tensor: torch.Tensor):
        bit, row, col = tensor.shape
        return tensor.transpose(1, 2).reshape(bit * 2, col // 2, row).transpose(1, 2)

    def split_neighbour(self, tensor: torch.Tensor):
        bit_width, row_half, col = tensor.shape
        return tensor.transpose(1, 2).reshape(bit_width // 2, col * 2, row_half).transpose(1, 2)
...
class wesco_v2_legacy(wesco_v2_base):
    ...
    def split_neighbour(self, tensor: torch.Tensor):
        bit_width, row, col = tensor.shape
...
class qmgr:
    ...
    def __init__(self, modules: Iterable[modulebase], printer):
        ...
        self.force_init: List[modulebase] = list()
    ...
    def process(self, tensor: torch.Tensor):
        if not self.state:
            ...
            for module in self.modules:
                module.once(self.state)
        else:
            self.state['tensor'] = tensor
            for module in self.force_init:
                module.once(self.state)
        
        self.state.pop('tensor')
