...
model: torch.nn.Module = LlamaForCausalLM.from_pretrained(model_name, **kwargs)

N = 65536
half_N = N // 2

all_stat = torch.zeros(N, dtype=torch.int32)
...
for name, m in model.named_modules():
    if isinstance(m, torch.nn.Linear) or isinstance(m, Conv1D):
        ...
        t = m.weight.data
        row = m.weight.data.numel() // 4
        t = t.view(row, 4)[:, 3]
        q = (t.float() * scale_0).int() + half_N
...
np.save('llama3_stat_col3.npy', all_stat)